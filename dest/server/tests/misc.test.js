...
describe('## Misc', function () {
  ...
  describe('# Error Handling', function () {
    it('should handle mongoose CastError - Cast to ObjectId failed', function (done) {
      (0, _supertestAsPromised2.default)(_index2.default).get('/api/donors/56z787zzz67fc').expect(_httpStatus2.default.INTERNAL_SERVER_ERROR).then(function (res) {
        (0, _chai.expect)(res.body.message).to.equal('Internal Server Error');
        done();
      });
    });

    it('should handle express validation error - donor id is required', function (done) {
      (0, _supertestAsPromised2.default)(_index2.default).post('/api/donors').send({
        mobileNumber: '+121234567890'
      }).expect(_httpStatus2.default.BAD_REQUEST).then(function (res) {
        (0, _chai.expect)(res.body.message).to.equal('"firstName" is required and "lastName" is required and "emailAddress" is required and "bloodGroup" is required and "latitude" is required and "longitude" is required and "address" is required');
        done();
      });
    });
  });
